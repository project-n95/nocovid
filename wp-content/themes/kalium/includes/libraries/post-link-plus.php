<?php
/**
 * Post link plus library.
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit; // Direct access not allowed.
}

// In case this plugin already exists
if ( function_exists( 'adjacent_post_link_plus' ) ) {
	return;
}
...
//		This prevents posts from being skipped when they have, for example, the same menu_order.
		...
		if ( false !== $result )
			return apply_filters( 'kalium_post_link_plus_result', $result ); // Modified by Arlind Nushi
...
//	When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
	foreach ( $posts as $post ) {
		...
		if ( empty($post->post_title) )
			$title = $previous ? __('Previous Post', 'kalium') : __('Next Post', 'kalium');
