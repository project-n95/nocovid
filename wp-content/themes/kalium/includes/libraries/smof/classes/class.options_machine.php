			...
			if ( ! empty( $smof_data ) || ! empty( $data ) ) {
				...
				switch ( $value['type'] ) {
					...
					case 'text':
						...
						$output .= '<input class="of-input ' . $mini . '" name="' . $value['id'] . '" id="' . $value['id'] . '" type="' . $value['type'] . '" value="' . $t_value . '" ' . ( isset( $value['plc'] ) ? ( ' placeholder="' . esc_attr( $value['plc'] ) . '"' ) : '' ) . ' />';
					...
					case 'heading':
						...
						$menu   .= '<li class="' . $header_class . '"><a title="' . $value['name'] . '" href="' . ( isset( $value['redirect'] ) ? $value['redirect'] : ( '#' . $jquery_click_hook ) ) . '"' . $icon . '><i class="header-icon' . ( isset( $value['icon'] ) ? " {$value['icon']}" : '' ) . '">' . kalium_get_svg_file( "assets/admin/images/theme-options/{$value['icon']}.svg", $value['icon'] ) . '</i><span>' . $value['name'] . '</span></a></li>';
...
//						$output .= '<input type="hidden" class="' . $fold . 'checkbox of-input" name="' . $value['id'] . '" id="' . $value['id'] . '" value="0"/>';
					...
					case 'content_builder':
						...
						if ( $conf['alignField'] ) {
							...
							$output .= sprintf( '<a href="#" class="button button-secondary set-menu-content-alignment"><i class="dashicons dashicons-editor-alignleft"></i></a>' );
							$output .= '<div class="menu-content-align-popover"><a href="#" data-align="left"><i class="dashicons dashicons-editor-alignleft"></i> <span>Left</span></a><a href="#" data-align="center"><i class="dashicons dashicons-editor-aligncenter"></i> <span>Center</span></a><a href="#" data-align="right"><i class="dashicons dashicons-editor-alignright"></i> <span>Right</span></a></div>';
							$output .= '</div>';
						}

						// Add menu content type
						$output .= '<div class="menu-content-button menu-content-button--add-entry">';
						$output .= sprintf( '<a href="#" class="button add-new-menu-content-entry" title="%1$s"><i class="dashicons dashicons-plus-alt"></i></a>', $conf['addEntryText'] );
						$output .= '</div>';
						...
						// Remove entry
						$output .= '<div class="of-input-menu-content-type--remove"><a href="#"><i class="dashicons dashicons-no-alt"></i></a></div>';
						...
						$output .= '<a href="#" class="menu-content-dropdown-icon options-show-popover" title="Options"><i class="dashicons dashicons-admin-generic"></i></a>';
					...
					case 'logo_image_display':
						...
						$output .= sprintf( '<img src="%1$s" class="logo-image-display-placeholder" alt="">', esc_attr( $image_url ) );
						break;
				}
	...
	public static function optionsframework_media_uploader_function( $id, $std, $mod ) {
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="' . $id . '">Upload <i class="dashicons dashicons-upload"></i></span>';
			...
			$uploader .= '<span class="button remove-image ' . $hide . '" id="reset_' . $id . '">Remove <i class="dashicons dashicons-no"></i></span>';
		} else {
