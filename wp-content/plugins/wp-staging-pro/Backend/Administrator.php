...
class Administrator
{
    ...
    private function defineHooks()
    {
        ...
        add_action("wp_ajax_wpstg_hide_disabled_mail_notice", [$this, "ajaxHideDisabledMailNotice"]);
    }
    ...
    public function ajaxHideDisabledMailNotice()
    {
        ...
        if (!class_exists('WPStaging\Backend\Pro\Notices\DisabledMailNotice')) {
            wp_send_json(null);
        }

        // @todo inject with dependency injection
        if ((new \WPStaging\Backend\Pro\Notices\DisabledMailNotice())->disable() !== false) {
            wp_send_json(true);
        }
